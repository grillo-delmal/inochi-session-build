...
SEMVER=$(semver ./src/inochi-session)
COMMIT=$(git -C ./src/inochi-session rev-parse HEAD)
GITVER=$(git_version ./src/inochi-session)
GITDIST=$(git_build ./src/inochi-session)

echo "%define inochi_session_ver ${GITVER}"
echo "%define inochi_session_semver ${SEMVER}"
echo "%define inochi_session_dist ${GITDIST}"
echo "%define inochi_session_commit ${COMMIT}"
echo "%define inochi_session_short ${COMMIT:0:7}"
echo ""
